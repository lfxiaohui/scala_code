...
//    var space_size = 268435456
    ...
    def main(args: Array[String]): Unit = {
        ...
        val ymd = args(0)
        val ymd_sta = args(1)
        sparkConf.set("spark.driver.maxResultSize", "8G").set("spark.debug.maxToStringFields", "100")
        ...
        val repartition = 500
        val train_output_path = "/user/xiaohui/fm_recall_coupon/"
        val feature = "S,Sc,Sd"
        ...
        val coupon_ctr =
            s"""
               ...
               |  AND obj_item_id <> '0'
               |  AND ymd >= '$ymd_sta' group by obj_coupon_id ) a where click < view
           """.stripMargin
...
//        var coupon_cvr_sql =
//            s"""
//            SELECT coupon_id,if(nvl(sum(click) / sum(view),0.0001)=0,0.0001,nvl(sum(click) / sum(view),0.0001)) as cvr from(
//               |select coupon_id,IF (label=0,1,0) as view,IF (label!=0,1,0) as click
//               | FROM alg.adm_sqkb_trainlog_hour
//               | WHERE log_type = 'home_feed'
//               | AND length(device_id) >= 14
//               | AND length(coupon_id) >= 2
...
//           """.stripMargin
    ...
    //提取序列特征
    def extractSequenceFeature(feaDic: Map[String, Any], feature: Array[String]): String = {
        val arr = feature.map(x => {
            var value = "-1"
            val key = x.split(":")(0)
            val fea = x.split(":")(1)
            if (feaDic.contains(key)) {
                value = feaDic(key).toString
            }
            splitSeq(value, fea)
        })
        arr.mkString(",")
    ...
    //序列特征编码
    def splitSeq(arr: String, fea: String): String = {
        val zero_arr = new Array[Int](50).map(_.toString)
        if (arr.isEmpty | arr == "-1") {
            zero_arr.map(x => fea + ":" + x).map(myhash).mkString(",") + ",0"
        } else {
            var arr_list = arr.split(",")
            val len = arr_list.size
            if (len < 50) {
                (arr_list ++ zero_arr.take(50 - len)).map(x => fea + ":" + x).map(myhash).mkString(",") + "," + len.toString
            } else {
                arr_list.take(50).map(x => fea + ":" + x).map(myhash).mkString(",") + "," + "50"
            }
        }
    }
